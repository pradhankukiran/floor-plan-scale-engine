 ...
 */
function expectWithinTolerance(actual, expected, tolerancePct, label) {
    const lower = expected * (1 - tolerancePct / 100);
    const upper = expected * (1 + tolerancePct / 100);
    expect(actual, `${label}: expected ${actual} to be within ${tolerancePct}% of ${expected} (range [${lower.toFixed(4)}, ${upper.toFixed(4)}])`).toBeGreaterThanOrEqual(lower);
    expect(actual, `${label}: expected ${actual} to be within ${tolerancePct}% of ${expected} (range [${lower.toFixed(4)}, ${upper.toFixed(4)}])`).toBeLessThanOrEqual(upper);
}
// ---------------------------------------------------------------------------
// Full pipeline tests
// ---------------------------------------------------------------------------
describe('computeScale — full pipeline', () => {
    ...
    it('should compute scale for a simple rectangle (bedroom)', async () => {
        ...
        expectWithinTolerance(result.pixelsPerInch, sample.expectedScale, TOLERANCE_PCT, sample.name);
    ...
    it('should compute scale for a polygon with a diagonal closing edge', () => {
        ...
        expectWithinTolerance(result.pixelsPerInch, 2.985, // ~675/226 ≈ ~394/132
        TOLERANCE_PCT, 'diagonal-closing-edge');
        expect(result.confidence).toBeGreaterThanOrEqual(0.5);
